...
void BitmapIO::WriteRGBAImageBufferAsImageBMP(const char* path, int width, int height, uint8_t* ptrPalette, uint8_t* ptrBuffer, double multiplier)
{
	...
	for (int i = 0; i < width * height; i++)
	{
		...
		truColorBuffer[truColorIdx + 0] = MultiplyValue(ptrPalette[ptrBuffer[(width * height) - 1 - i] * 3 + 2], multiplier);
		truColorBuffer[truColorIdx + 1] = MultiplyValue(ptrPalette[ptrBuffer[(width * height) - 1 - i] * 3 + 1], multiplier);
		truColorBuffer[truColorIdx + 2] = MultiplyValue(ptrPalette[ptrBuffer[(width * height) - 1 - i] * 3], multiplier);
...
void BitmapIO::WritePosistructToAlphaPng(uint8_t* pallettebuffer, uint8_t* buffer, int width, int height, const char* filename, char* title, int padding) {
	char textbuffer[512];
	std::vector<uint8_t> buffer2((width + (2 * padding)) * (height + (2 * padding)) * 4);
	...
	for (int y = 0; y < height + (2 * padding); y++)
	{
		for (int x = 0; x < width + (2 * padding); x++)
		{
			...
			int y2 = y - padding;
			if ((x < padding) || (y < padding) || (x >= width + padding) || (y >= height + padding)) {
				buffer2[(y * (width + (2 * padding)) + x) * 4 + 0] = 255;
				buffer2[(y * (width + (2 * padding)) + x) * 4 + 1] = 255;
				buffer2[(y * (width + (2 * padding)) + x) * 4 + 2] = 255;
				buffer2[(y * (width + (2 * padding)) + x) * 4 + 3] = 255;
			}
			else
			{
				...
				if (buffer[y2 * width + x2] == TRANSPARENT_COLOUR)
				{
					buffer2[(y * (width + (2 * padding)) + x) * 4 + 0] = 0;
					buffer2[(y * (width + (2 * padding)) + x) * 4 + 1] = 0;
					buffer2[(y * (width + (2 * padding)) + x) * 4 + 2] = 0;
					buffer2[(y * (width + (2 * padding)) + x) * 4 + 3] = 255;
				}
			}
		}
